...
class PricePredictionService extends BasePricePredictionService implements IPricePredictionService
{
    ...
    ): ServiceResponse
    {
        ...
        $parameters = [
            ...
            'ml' => roundFrom($kilometerFrom) . ':' . roundTo($kilometerTo),
            'fr' => $yearFrom . ':' . $yearTo,
            'ft' => implode(' ', $targetFuelTypes),
            'tr' => implode(' ', $targetGearBoxes),
            'powertype' => 'kw',
            'pw' => roundPowerFrom($powerFrom) . ':' . roundPowerTo($powerTo),
            'c' => $targetBodyType,
            'cn' => 'DE',
        ...
        //$chromeOptions->addArguments(['--disable-extensions','--ignore-certificate-errors','--no-sandbox','--disable-gpu', '--window-size=1920,1080', '--disable-dev-shm-usage']);
        
        //$chromeOptions->addArguments(['--no-sandbox','--profile-directory=Profile 1','user-data-dir=+userProfile','--crash-dumps-dir=/tmp','--remote-debugging-port=9222','--disable-setuid-sandbox','--incognito','--remote-debugging-port=9222','--disable-infobars','--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.5060.114 Safari/537.36 OPR/89.0.4447.64','--window-size=1920,1080', '--disable-dev-shm-usage']);
        $capabilities = DesiredCapabilities::chrome(); // 'user-data-dir=usr/bin/google-chrome','--profile-directory=Default'
        ...
        if (count($priceList) < 10) {
            ...
            $parameters = [
                ...
                'powertype' => 'kw',
                'powerfrom' => roundPowerFrom($powerFrom),
                'powerto' => roundPowerTo($powerTo),
                'body' => $targetBodyType,
                //'sort' => 'mileage',
                //'desc' => '0',
                'sort' => 'price',
